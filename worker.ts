...
function getProductId(payload: ShopifyOrderPayload): string | null {
...
function getCustomerEmail(payload: ShopifyOrderPayload): string | null {
...
async function createUniqueCode(env: Env, productType: string, email: string): Promise<string> {
...
async function handleWebhook(request: Request, env: Env): Promise<Response> {
    ...
    try {
        payload = await request.json() as ShopifyOrderPayload
    } catch {
    ...
    try {
        ...
        if (!updateResult) {
            const record = await env.DB.prepare(
                ...
                .bind(code)
                .first<{ code: string; used: number | boolean }>()
